...
namespace Muserank2
{
    public partial class MuseService
    {
        ...
        public async Task ExportChartsToExcel2(Query query = null, string fileName = null)
        {
            navigationManager.NavigateTo(query != null ? query.ToUrl($"export/muse/charts2/excel(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')") : $"export/muse/charts2/excel(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')", true);
        }

        public async Task ExportChartsToCSV2(Query query = null, string fileName = null)
        {
            navigationManager.NavigateTo(query != null ? query.ToUrl($"export/muse/charts2/csv(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')") : $"export/muse/charts2/csv(fileName='{(!string.IsNullOrEmpty(fileName) ? UrlEncoder.Default.Encode(fileName) : "Export")}')", true);
        }
        ...
        public async Task<IQueryable<Models.Muse.Chart>> GetCharts2(Query query = null)
        {
            var items = context.Charts.OrderBy(x => x.AUTHOR_NAME).AsEnumerable<Models.Muse.Chart>().AsQueryable();
